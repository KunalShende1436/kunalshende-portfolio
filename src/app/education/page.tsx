import { getEducation } from '@/services'
import EducationSection from '@/components/Education/EducationSection'
import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Education | Kunal M Shende',
  description: 'My academic background in Computer Science & Engineering and high school qualifications.',
}

export default async function EducationPage() {
  const education = await getEducation()

  return (
    <main className="mx-auto my-8 max-w-[1000px] px-4 md:my-[3.75rem] min-h-[70vh]">
      <EducationSection education={education} showSeeMore={false} />

      <div className="mt-16 text-center">
        <a
          href="/"
          className="text-accent hover:text-white transition-colors duration-300 font-semibold"
        >
          ← Back to Homepage
        </a>
      </div>
    </main>
  )
}
