  ...
  return (
    <nav className="bg-primary border-border relative h-16 border-b">
      ...
      <div className="mx-auto flex h-full w-dvw max-w-[1200px] items-center justify-between px-4 py-1">
        {isVisible ? (
          <div className="text-primary-content md:hidden font-bold tracking-wider">MENU</div>
        ) : (
          <a href="/" className="flex-shrink-0">
            <div className="animate-fade-up text-primary-content relative flex items-center gap-3 transition-all duration-300 md:static">
              <Logo />
              <span className="text-primary-content hover:text-accent cursor-pointer text-lg font-bold tracking-tight transition-colors duration-300 whitespace-nowrap select-none">
                KUNAL M SHENDE
              </span>
            </div>
          </a>
          ...
          {navItems.map(({ label, href }) => {
            return (
              <li
                ...
                <Link
                  href={href}
                  className={`text-primary-content hover:text-neutral w-full py-4 transition-all duration-150 md:py-0 ${pathname === href ? 'text-neutral font-bold text-accent' : ''}`}>
                  {label}
                </Link>
              </li>
            )
